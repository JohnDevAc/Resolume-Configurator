...
When Configure is pressed, the app overwrites Arena's currently open composition without saving it. Because Arena 7.27 rejects REST deletion requests in some blank compositions, the configurator uses a deletion-free build: it reuses existing groups and layers, moves ungrouped or surplus layers into the required groups, adds only missing structure, clears clips, and renames the final graph. Unsaved changes in the open composition are discarded. Routing is deferred until the complete layer/group structure has been re-read from Arena. Advanced Output is not exposed by Resolume's REST API, so the app writes both a named XML preset under `Resolume Arena\Presets\Advanced Output` and Arena's active `Preferences\AdvancedOutput.xml` state directly. The active file is backed up and replaced atomically; no menu selection or window automation is used.
